import React from "react";


const FlyIcons = () => {
    return (
        <div className="half-column relative top-line integrations-wrapper">
            <div className="integration-icon zoominfo">
                <img
                    src="https://cdn0.iconfinder.com/data/icons/social-network-7/50/22-512.png"
                    alt="ZoomInfo logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://cdn0.iconfinder.com/data/icons/social-network-7/50/22-512.png"
                        alt="ZoomInfo logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon marketo">
                <img
                    src="https://cdn-icons-png.flaticon.com/512/331/331383.png"
                    alt="Marketo logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://cdn-icons-png.flaticon.com/512/331/331383.png"
                        alt="Marketo logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon salesforce">
                <img
                    src="https://cdn.iconscout.com/icon/free/png-512/javascript-2752148-2284965.png"
                    alt="Salesforce logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                    width="57"
                />
                <noscript>
                    <img
                        src="https://cdn.iconscout.com/icon/free/png-512/javascript-2752148-2284965.png"
                        alt="Salesforce logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon chrome">
                <img
                    src="https://www.zoominfo.com/wp-content/uploads/2020/11/chrome.svg"
                    alt="Chrome logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://www.zoominfo.com/wp-content/uploads/2020/11/chrome.svg"
                        alt="Chrome logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon salesloft">
                <img
                    src="https://upload.wikimedia.org/wikipedia/commons/4/49/Redux.png"
                    alt="salesloft logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://upload.wikimedia.org/wikipedia/commons/4/49/Redux.png"
                        alt="salesloft logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon ringlead">
                <img
                    src="https://cdn-icons-png.flaticon.com/512/919/919851.png?pad=10,10,10,10&ext=png&bg=FFFFFFFF"
                    alt="ringlead logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://cdn-icons-png.flaticon.com/512/919/919851.png?pad=10,10,10,10&ext=png&bg=FFFFFFFF"
                        alt="ringlead logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon outreach">
                <img
                    src="/-Yair-Levi-14-03-2022//2725760.png"
                    alt="outreach logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="/-Yair-Levi-14-03-2022//2725760.png"
                        alt="outreach logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon microsoft">
                <a href="tel:+972527249942">
                    <img
                        src="/-Yair-Levi-14-03-2022//2936030.png"
                        alt="microsoft logo"
                        className="lazyloaded"
                        data-ll-status="loaded"
                    />
                    <noscript>
                        <img
                            src="/-Yair-Levi-14-03-2022//2936030.png"
                            alt="microsoft logo"
                        />
                    </noscript>
                </a>
            </div>
            <div className="integration-icon eloqua">
                <img
                    src="https://cdn-icons-png.flaticon.com/128/2721/2721155.png"
                    alt="eloqua logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://cdn-icons-png.flaticon.com/128/2721/2721155.png"
                        alt="eloqua logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon hubspot">
                <img
                    src="https://cdn-icons-png.flaticon.com/512/2076/2076808.png"
                    alt="hubspot logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://cdn-icons-png.flaticon.com/512/2076/2076808.png"
                        alt="hubspot logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon snowflake">
                <a href="mailto:hthrkuh8@gmail.com">
                    <img
                        src="/-Yair-Levi-14-03-2022//3296464.png"
                        alt="snowflake logo"
                        className="lazyloaded"
                        data-ll-status="loaded"
                    />
                    <noscript>
                        <img
                            src="/-Yair-Levi-14-03-2022//3296464.png"
                            alt="snowflake logo"
                        />
                    </noscript>
                </a>
            </div>
            <div className="integration-icon stack">
                <img
                    src="https://www.zoominfo.com/wp-content/uploads/2021/02/stack-logo-5.svg"
                    alt="stack logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://www.zoominfo.com/wp-content/uploads/2021/02/stack-logo-5.svg"
                        alt="stack logo"
                    />
                </noscript>
            </div>
            <div className="integration-icon bullhorn">
                <img
                    src="https://pic2.zhimg.com/v2-9fc9ae8f1d0e95aed316a92f71dc4ba1_xs.jpg?source=172ae18b"
                    alt="bullhorn logo"
                    className="lazyloaded"
                    data-ll-status="loaded"
                />
                <noscript>
                    <img
                        src="https://pic2.zhimg.com/v2-9fc9ae8f1d0e95aed316a92f71dc4ba1_xs.jpg?source=172ae18b"
                        alt="bullhorn logo"
                    />
                </noscript>
            </div>
        </div>
    );
};
export default FlyIcons